        ...
        <div class="crumbs">
            <el-form :inline="true" class="demo-form-inline" style="text-align:left">
                <el-form-item label="名称">
                    <el-input placeholder="图书名称" v-model="searchForm.name"></el-input>
                </el-form-item>
  ...
  export default {
    ...
    data () {
      return {
        ...
        tableConfigMsg: {
          tableTitle: [
            { name: 'name', label: '名称', align: 'center', width: '' },
            { name: 'type', label: '类型', align: 'center', width: '180' },
            { name: 'price', label: '薪酬', align: 'center', width: '100' },
    ...
    methods: {
      getData () {
        this.$store.dispatch('updateTutoring')
      },
      // 类型转换
      toType (type) {
        let result = type
      ...
      handleEdit (index, row) {
        ...
        // 保存产品信息
        const aProductMsg = JSON.parse(JSON.stringify(this.dialogMsg.tutoringMsg))
