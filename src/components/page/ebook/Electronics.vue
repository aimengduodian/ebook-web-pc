        ...
        <div class="crumbs">
            <el-form :inline="true" class="demo-form-inline" style="text-align:left">
                <el-form-item label="电子名称">
                    <el-input placeholder="电子名称" v-model="searchForm.electronicsName"></el-input>
                </el-form-item>

                <el-form-item label="电子类型">
                    <el-select v-model="searchForm.electronicsType" clearable placeholder="请选择">
  ...
  export default {
    ...
    data () {
      return {
        ...
        tableConfigMsg: {
          tableTitle: [
            { name: 'electronicsName', label: '电子名称', align: 'center', width: '' },
            { name: 'electronicsType', label: '电子类型', align: 'center', width: '180' },
            { name: 'presentPrice', label: '出售价格', align: 'center', width: '100' },
    ...
    methods: {
      getData () {
        this.$store.dispatch('updateElectronics')
      },
      // 类型转换
      toType (type) {
        let result = type
      ...
      handleEdit (index, row) {
        ...
        // 保存图片信息
        let pic = row.electronicsPic.split(',')
        this.dialogMsg.pics = []
        ...
        // 保存产品信息
        const aProductMsg = JSON.parse(JSON.stringify(this.dialogMsg.electronicsMsg))
      ...
      handleDelete (index, row) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          fetchDeleteElectronics(data).then(res => {
            if (res.code === 100) {
